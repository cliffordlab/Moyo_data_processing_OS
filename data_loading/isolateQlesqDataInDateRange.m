function qlesqOut = isolateQlesqDataInDateRange(qlesq, startDate, varargin)
% 
% Overview
%	Returns qlesq data between start and end dates.
% 
...
idxWithinDateRange = find(qlesq.t >= startDate & qlesq.t < endDate);

% Isolate just the values within the date range, and save to output variables
qlesqOut.t = qlesq.t(idxWithinDateRange);
qlesqOut.val = qlesq.val(idxWithinDateRange);

function painSwipeOut = isolatePSDataInDateRange(painSwipe, startDate, varargin)
% 
% Overview
%	Returns PS data between start and end dates.
% 
...
idxWithinDateRange = find(painSwipe.t >= startDate & painSwipe.t < endDate);

% Isolate just the values within the date range, and save to output variables
painSwipeOut.t = painSwipe.t(idxWithinDateRange);
painSwipeOut.val = painSwipe.val(idxWithinDateRange);

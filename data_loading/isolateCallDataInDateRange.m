function callOut = isolateCallDataInDateRange(Call, startDate, varargin)
% 
% Overview
%	Returns call data between start and end dates.
% 
...
idxWithinDateRange = find(Call.t >= startDate & Call.t < endDate);

% Isolate just the values within the date range, and save to output variables
if ~isempty(idxWithinDateRange)
    callOut.t = Call.t(idxWithinDateRange);
    callOut.id = Call.id(idxWithinDateRange);
    callOut.type = Call.type(idxWithinDateRange);
    callOut.dur = Call.dur(idxWithinDateRange);
else
    callOut.t = [];
    callOut.id = [];
    callOut.type = [];

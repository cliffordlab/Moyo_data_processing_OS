function locationOut = isolateLocDataInDateRange(Location, startDate, varargin)
% 
% Overview
%	Returns location data between start and end dates.
% 
...
idxWithinDateRange = find(Location.t >= startDate & Location.t < endDate);

% Isolate just the values within the date range, and save to output variables
locationOut.t = Location.t(idxWithinDateRange);
locationOut.lat = Location.lat(idxWithinDateRange);
locationOut.lon = Location.lon(idxWithinDateRange);
